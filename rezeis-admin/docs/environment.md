...
| Переменная | Назначение | Обяз. | Прод |
|---|---|---|---|
| `WEBHOOK_ENABLED` | Включает доставку системных событий на внешний URL — всех, кроме адресованных только оператору (`payment.withheld`, `payment.withheld_refunded`: платёж получен, но не применён, и его возврат). Они видны только в «Журнал аудита» и в карточке в Telegram-группе операторов. | — | `false`, если нет внешнего приёмника |
