...
| Переменная | Назначение | Обяз. | Прод |
|---|---|---|---|
| `REMNAWAVE_HOST` | Хост панели Remnawave. Имя сервиса (без точки) или **частный** адрес (loopback, `10.`/`192.168.`/`172.16-31.`, `localhost`) → `http://host:port`. Всё, что доступно из интернета — домен или маршрутизируемый IP → `https://host`. Литерал адреса доменом не считается: точки в нём есть, а сертификата на него нет. | да | `remnawave` (одна сеть) или домен |
| `REMNAWAVE_PORT` | Порт. Используется только для частного адреса; для домена и публичного IP игнорируется — при старте будет строка в журнале. | — | `3000` |
...
> Ранее здесь значились `REMNAWAVE_CADDY_TOKEN` и `REMNAWAVE_COOKIE`. Они
> объявлялись в схеме, но ни один запрос к панели их не использовал: заголовок
> и cookie никогда не собирались. Переменные удалены, чтобы не обещать
> оператору поведение, которого нет. Если Remnawave стоит за caddy-security,
> поддержку нужно реализовывать заново — вместе с кодом, который шлёт заголовок.
...
### Web Push (VAPID) — настраивается только в панели

Переменных окружения для web-push больше нет. Ключи генерируются в админке:
**Настройки → Web-push → «Сгенерировать ключи»**; приватный ключ хранится в базе
зашифрованным.

`VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_CONTACT_EMAIL` при отправке
больше **не читаются**. Если они остались в `.env` со старой версии, первая
загрузка после обновления скопирует **ту же самую** пару ключей в панель (чтобы
не потерять уже оформленные подписки браузеров), после чего их следует удалить
из `.env`. Задавать их на новом развёртывании бессмысленно.

Если ключей нет нигде, панель говорит об этом сама: событие
`system.web_push_unconfigured` (в журнале, в реальном времени и в Telegram
операторам) плюс предупреждение на карточке «Web-push» в настройках. До этого
момента любая отправка push — пустая операция.
