...
Этапы нового учёта докупок включаются в панели: «Доп. услуги» → вкладка
«Настройки». Строки `ADDON_*` для этапов в `.env` не нужны; уже вписанная
строка продолжает действовать и важнее переключателя (подробно —
[`docs/operator-add-on-entitlements-rollout.md`](./operator-add-on-entitlements-rollout.md)).

| Переменная | Назначение | Прод |
|---|---|---|
| `ADDON_SLO_OBJECTIVE_MS` / `ADDON_SLO_ALERT_MS` | Пороги отставания в миллисекундах в `GET /admin/add-on-entitlements/metrics` — оплаченные, но не выданные докупки и неотправленные изменения: цель и тревога. По умолчанию `300000` / `900000`. | не задавать |
