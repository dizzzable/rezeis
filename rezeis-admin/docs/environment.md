...
| Переменная | Назначение | Обяз. | Прод |
|---|---|---|---|
| `WEBHOOK_ENABLED` | Включает доставку системных событий на внешний URL — всех, кроме адресованных только оператору: `payment.withheld` и `payment.withheld_refunded` (платёж получен, но не применён, и его возврат), `payment.chargeback_unmatched` (оспорено списание, которое панель не нашла) и `payment.autopay_stopped_by_operator` (оператор отключил клиенту автосписание в панели). Они видны только в самой панели («Журнал аудита», открытые страницы обновляются сами) и в карточке в Telegram-группе операторов. | — | `false`, если нет внешнего приёмника |
...
### Докупки: новый учёт (`ADDON_*`)

Новый учёт докупок: у каждой подписки есть сроки, а докупка из кабинета
(«Дополнительные опции») записывается с датой окончания, а не поднимает лимит
навсегда. Все переменные необязательны — не задана, значит значение по
умолчанию. `true`, `1`, `on`, `yes` включают этап, `false`, `0`, `off`, `no` —
выключают (регистр и пробелы не важны). Любое другое значение оставляет
значение по умолчанию и пишет одно предупреждение в журнал: `disabled` этап
**не** выключает. Оба контейнера читают `.env`, изменение вступает в силу после
`docker compose up -d`. Что делает каждый этап, как его выключить и что
выключение **не** отменяет — в
[`docs/operator-add-on-entitlements-rollout.md`](./operator-add-on-entitlements-rollout.md).

| Переменная | По умолчанию | Назначение |
|---|---|---|
| `ADDON_ENTITLEMENT_SHADOW` | `true` | Этап 1: подписки переходят в новый учёт. Задача в воркере переводит все существующие подписки (до 5 000 за проход, проход раз в 5 минут, первый — при запуске), новые подписки создаются сразу в нём. Лимиты трафика и устройств подписки в новом учёте ведёт панель: лимит, изменённый прямо в Remnawave, панель возвращает. Меняйте лимиты в панели. |
| `ADDON_ENTITLEMENT_DIRECT_PURCHASE` | `true` | Этап 2: докупка записывается с датой окончания — до конца подписки. Выключен — докупка поднимает лимит навсегда, как раньше. |
| `ADDON_DEVICE_CLEANUP_AUTO` | `true` | Этап 6: когда заканчивается докупка устройств, лишние зарегистрированные устройства удаляются сами, начиная с самых новых. Выключен — сокращение ждёт утверждения оператора. |
| `ADDON_PROJECTION_SYNC` | `false` | Этап 3: версионная отправка в Remnawave с проверкой чтением. **Не включать**: она не передаёт срок, и платное улучшение тарифа не сдвинуло бы срок в Remnawave. |
| `ADDON_RESET_EXPIRY_DAY` / `_WEEK` / `_MONTH` / `_MONTH_ROLLING` | `false` | Этап 4: докупки «до следующего сброса трафика», по одной переменной на стратегию сброса. **Не включать**, пока не показано совпадение с вашей версией Remnawave (см. руководство). |
| `ADDON_RENEWAL_ADDONS` | `false` | Этап 5: докупки вместе с оплатой продления. Выключен решением владельца. |
| `ADDON_SLO_OBJECTIVE_MS` / `ADDON_SLO_ALERT_MS` | `300000` / `900000` | Пороги отставания (миллисекунды) в `GET /admin/add-on-entitlements/metrics` — оплаченные, но не выданные докупки и неотправленные изменения: цель и тревога. |
...
| `REIWA_WEB_BASE_URL` | Запасной публичный адрес кабинета (`https://…`). Адрес кабинета панель берёт сначала у самого кабинета (`<REIWA_URL>/api/v1/public-config`, его `REIWA_DOMAIN`; если кабинет перестал отвечать — последний адрес, который он публиковал, пока панель не перезапущена), затем отсюда, затем из `MINIAPP_CUSTOM_URL`. На этом адресе строится всё, что видит клиент: ссылка внизу каждого письма, кнопка «Открыть переписку» в письме гостю поддержки, логотип в письмах, готовые рекламные ссылки, страница возврата после оплаты (когда кабинет не передал свою) и адрес покупателя `payment-…@<хост>` в счетах Antilopay, SeverPay и Lava. | нет | обычно не задавать; задать, если кабинет не отвечает панели по `REIWA_URL`: `https://app.example.com` |
| `MINIAPP_CUSTOM_URL` | Адрес Mini App — второй запасной адрес кабинета, после `REIWA_WEB_BASE_URL`, для всего того же. | нет | не задавать, если задан `REIWA_WEB_BASE_URL` |

> **Если адреса кабинета нет нигде** — кабинет его не опубликовал (или недоступен
> панели), а `REIWA_WEB_BASE_URL` и `MINIAPP_CUSTOM_URL` не заданы:
> - в письмах нет ссылки внизу, а в письме гостю нет кнопки «Открыть переписку» —
>   письмо говорит, что ответ ждёт в чате поддержки на сайте;
> - загруженный логотип письма грузится с `REZEIS_DOMAIN` (если там `localhost`
>   или имя docker-сервиса — письмо уходит без логотипа);
> - готовых рекламных ссылок нет, остаются payload `ad_<код>` и код отслеживания;
> - платежи не отказывают, но плательщика, которому кабинет не передал свою
>   страницу возврата, провайдер вернёт на `<REZEIS_DOMAIN>/payments/result` —
>   такой страницы нет нигде, он увидит «не найдено» админки; адрес покупателя в
>   счетах Antilopay, SeverPay и Lava — `payment-…@<домен панели>`. В журнале
>   панели — одно предупреждение за запуск. Чтобы этого не было, задайте
>   `REIWA_WEB_BASE_URL` — публичный адрес кабинета.
>
> В письмах адрес панели (`REZEIS_DOMAIN`) ссылкой не появляется никогда;
> плательщик видит его только в случае выше, когда адреса кабинета нет.
...
| Переменная | По умолчанию | Назначение |
|---|---|---|
| `SUPPORT_GUEST_TOKEN_TTL_HOURS` | `72` | Сколько часов гость может открыть свою переписку: с начала обращения, а каждый ответ оператора продлевает доступ ещё на столько же от этого ответа. Если оператор не отвечал дольше — вернуться к переписке нельзя. Сообщения самого гостя доступ не продлевают. Закрытое обращение не продлевается. Задаётся в панели: «Тикеты поддержки» → «Настройки» → «Срок жизни токена гостя (часы)»; это значение — только начальное. |
