      ...
      metadata: {
        ...
        ...(stranded
          ? {
              ...
              nextSteps:
                'Причина — в «💬 Сообщение» ниже; после её исправления перезапустите панель, и ключи ' +
                'перенесутся сами. Или сгенерируйте новые на странице «Платформа», в карточке «Web-push ' +
                '(браузерные уведомления)» → «Сгенерировать ключи»: прежние подписки браузеров станут ' +
                'недействительны, и кабинет переподпишется сам при следующем открытии.',
            }
          : {}),
      },
    });
