...
interface AppSeed {
  ...
  /**
   * The app's own mark, by key into `ICONS`.
   *
   * Absent is legitimate and renders the fallback glyph: an app nobody has a
   * mark for is still an app that connects. What is NOT legitimate is a key
   * with no icon behind it, which the audit already refuses.
   */
...
/**
 * The icon library the catalog draws from.
 *
 * Two kinds, and the difference is load-bearing:
 *
 *   - STEP AND PLATFORM icons are stroke drawings on `currentColor`, so they
 *     take the accent of whatever concept the screen is wearing. They have no
 *     palette of their own and must not acquire one.
 *   - APPLICATION MARKS carry their own colours, because a brand mark that
 *     changes colour is not that brand's mark. Most of them are a light glyph
 *     with no ground of their own, which is why the cabinet draws them on a
 *     plate — on a light concept an unbacked white glyph is invisible.
 *
 * Everything here is stored as authored and served as authored. That is safe
 * only because it is verified: `connect-page-default.spec.ts` runs every entry
 * through the same sanitizer an operator's upload goes through and fails if any
 * of them loses a single byte. Adding one by hand without that check is how a
 * default ships something an upload would have been refused for.
 */
const ICONS: Record<string, string> = {
  ...
  // Platforms. Stroke icons on `currentColor`, like the five above.
  'apple':
  ...
  // From the operator's own exported subscription page — the five its library
  // had that ours did not. Everything else in that export is the same drawing
  // we already ship under our own slug (`ClashMeta` -> `clash-meta`), which is
  // why the importer matches on the drawing rather than on the name.
  'gear':
    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10.325 4.317c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.724 1.724 0 0 0 1.065 2.572c1.756 .426 1.756 2.924 0 3.35a1.724 1.724 0 0 0 -1.066 2.573c.94 1.543 -.826 3.31 -2.37 2.37a1.724 1.724 0 0 0 -2.572 1.065c-.426 1.756 -2.924 1.756 -3.35 0a1.724 1.724 0 0 0 -2.573 -1.066c-1.543 .94 -3.31 -.826 -2.37 -2.37a1.724 1.724 0 0 0 -1.065 -2.572c-1.756 -.426 -1.756 -2.924 0 -3.35a1.724 1.724 0 0 0 1.066 -2.573c-.94 -1.543 .826 -3.31 2.37 -2.37c1 .608 2.296 .07 2.572 -1.065z"/><path d="M9 12a3 3 0 1 0 6 0a3 3 0 0 0 -6 0"/></svg>',
  'plus':
    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 5v14"/><path d="M5 12h14"/></svg>',
  'tv':
...
export const DEFAULT_CONNECT_PAGE_CONFIG: ConnectPageConfig = {
  ...
  // No concept of its own: the screen wears whatever the cabinet wears. Both
    ...
    },
    {
      ...
      apps: [
        ...
        app({
          id: 'v2rayn',
          name: 'v2rayN',
          // The 2dust family mark, shared with v2rayNG — which is why the key
          // is named for the family and not for one of its members.
          iconKey: 'v2ray',
