            ...
            `партнёра ${context.partnerId} после сбоя выдачи — требуется ручной возврат`,
      {
        ...
        nextSteps: durablyRecorded
          ? 'Обычно делать ничего не нужно. Не корректируйте баланс вручную, пока долг записан: ' +
            ...
            'возврат каждый раз падает: причина — в логах панели, строка «Partner-balance refund retry failed».'
          : `Верните деньги вручную: «Пользователи» → этот пользователь → вкладка «Партнёр» → «Корректировка ` +
            `баланса»: сумма +${formatMinorUnits(context.amountMinor)}, в причине — платёж ${context.paymentId}, затем «Применить».`,
      },
    );
    this.logger.error(
      `Partner-balance refund of ${context.amountMinor} to partner ${context.partnerId} ` +
      ...
      this.events.error(
          ...
          'быть выполнен автоматически — требуется ручной возврат',
        {
          ...
          nextSteps:
            ...
            'вручную: «Пользователи» → этот пользователь → вкладка «Партнёр» → «Корректировка баланса»: сумма, ' +
            'затем «Применить».',
        },
      );
      this.logger.error(
