...
export class InternalUserDevicesController {
  ...
  public async deleteSubscriptionDevice(
    ...
    // The same refusal as `deleteDevice`, spelled out at this site rather than
  ...
  public async regenerateSubscription(
    ...
    // re-issue the old value. This is the one verb in the system with no way
    // back — a deletion can be re-provisioned and a device slot can be re-bound,
    ...
    // instead of silently doing the wrong thing. The cost of refusing is one
    ...
    this.events.error(
      ...
      `Subscription link regenerated on the panel but NOT stored — the saved link is dead until a panel sync (${reason})`,
      {
        ...
        why:
          ...
          'подключается.',
        nextSteps:
          'Откройте «Пользователи» → этого пользователя → вкладку «Подписки» и нажмите у подписки ' +
          'значок ↻ («Синхронизировать»): он перечитает ссылку из Remnawave. После этого подписчику нужно ' +
