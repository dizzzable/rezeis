  ...
  private async revive(broadcastId: string, why: string, kind: 'schedule' | 'stalled'): Promise<void> {
    ...
    this.systemEvents.warn(
      ...
      `Broadcast ${broadcastId} was picked up again: ${why}`,
      {
        ...
        why:
          kind === 'schedule'
            ? 'Запланированная рассылка не стартовала в срок: в очереди нет задачи на её запуск. ' +
              'Её поставили в очередь заново — отправка начнётся сама. Вмешиваться не нужно; если ' +
