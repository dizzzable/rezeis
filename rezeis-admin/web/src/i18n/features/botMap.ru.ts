...
export const ru = {
  botMapPage: {
    ...
    destination: {
      ...
      unanswered: '✕ Бот не знает эту кнопку — ответит «Меню обновилось» и покажет главное меню',
      missingScreen: '✕ Такого экрана нет — бот ответит «Меню обновилось» и покажет главное меню',
    ...
    notification: {
      ...
      lifetimeTopUpHint:
        'У бессрочной подписки продлевать нечего, поэтому кнопки этого уведомления, ведущие на «Продление подписки» (/renew), бот заменяет на «📦 Докупить трафик» → «Дополнения» (/addons) этой подписки — если ей есть что там купить: докупку трафика или «Сброс трафика». Если купить нечего (например, бессрочная подписка на тарифе без сброса трафика и без «Сброса трафика» в каталоге), таких кнопок бот не показывает, а само уведомление приходит. Push ведёт туда же, а если купить нечего — на главную; колокольчик в кабинете тогда просто показывает уведомление. На схеме — отдельная стрелка.',
  ...
  botFlow: {
    screenTexts: {
      ...
      // Под полем текста, который Telegram берёт не длиннее своего предела
      // («Меню обновилось» — всплывающее сообщение, 200 символов).
      tooLong:
        'Не сохранится: Telegram показывает этот текст всплывающим сообщением и берёт не больше {{max}} символов, а здесь {{count}}. Сократите текст.',
      // Текст, который бот шлёт и обычным сообщением, и всплывающим на нажатие
      // кнопки («Вы ещё не подписаны», отказы режима доступа, задание-канал).
      tooLongAlsoMessage:
        'Не сохранится: бот показывает этот текст и всплывающим сообщением, а его Telegram берёт не больше {{max}} символов, а здесь {{count}}. Сократите текст.',
      alsoPopupHint:
        'Этот текст бот показывает и обычным сообщением, и всплывающим — при нажатии кнопки. Всплывающее сообщение Telegram берёт не больше {{max}} символов, поэтому и этот текст не длиннее {{max}}.',
      ...
      captions: {
        ...
        menuUpdated:
          'Старая кнопка нажата — всплывающее сообщение над главным меню, которое бот рисует на месте того сообщения',
      },
    },
    systemButtons: {
    ...
    replyTargets: {
      ...
      site: 'сайт кабинета {{path}}',
      url: '{{host}}',
      unhandled: 'бот не знает эту кнопку — ответит «Меню обновилось» и покажет главное меню',
      mainMenu: 'главное меню',
      missingScreen: 'нет экрана {{shortId}} — бот ответит «Меню обновилось» и покажет главное меню',
    ...
    systemScreens: {
      ...
      staleButton: {
        title: 'Старая кнопка',
        trigger:
          'Нажата кнопка из старого сообщения, которой в боте больше нет: её убрали или поменяли, или удалили экран, на который она вела. Бот показывает всплывающее «Меню обновилось» и рисует главное меню на месте этого сообщения. Старые кнопки бот не перенаправляет, а сообщения, которых не касались, не переписывает',
      },
      serviceReplies: {
      ...
      captions: {
        ...
        menuUpdated: 'Всплывающее сообщение над главным меню',
      },
    },
  },
  ...
  botConfigPage: {
    ...
    // Вкладка «Тексты»: стандартный текст панель записывает снова при
    // следующем запуске, так что удалить его — значит вернуть к стандартному.
    texts: {
      resetToDefault: 'Вернуть стандартный текст',
      resetToDefaultHint:
        'Это стандартный текст бота. Кнопка ниже вернёт его к стандартному — и русский, и английский: бот сразу покажет стандартный текст, а после перезапуска панели он снова появится в этом списке. Ваш вариант не сохранится.',
      toasts: {
        resetToDefault: 'Текст возвращён к стандартному',
      },
    },
  },
} as const
