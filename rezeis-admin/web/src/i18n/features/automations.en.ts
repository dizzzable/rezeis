...
export const en = {
  automationsPage: {
    ...
    runResults: {
      ...
      audience_blind_signal: 'The hint was queued for nobody: the panel cannot tell right now who has connected. The connection check has not been able to read Remnawave for half an hour, and webhooks alone are not enough: a customer who connected but whose webhook never arrived would look not connected and get the hint for nothing. Check how the panel connects to Remnawave; the rule tries again on its next run.',
    ...
    hintTemplates: {
      ...
      connect_help: {
        ...
        info: 'The subscription.not_connected event is raised by the "Connection help" card on the Notifications page, User tab. It happens only while "Send automatically" is on there, and for trials and gifts only with "Also trials and gifts" ticked. It comes once per subscription and after the attempt to message the customer, so the customer sees the window after the message — when they open the cabinet.\n\nThe window does not appear if by then the customer has no live subscription left whose VPN never connected, or has switched connection help off in their cabinet. It is shown only after a fresh check — no older than 15 minutes — that the VPN still has not connected; the cabinet makes that check itself when the customer opens it. Until there is one, the window waits and the customer sees the hints after it. An older cabinet does not show such a window: it waits for the cabinet to be updated until the hint expires.',
