...
/**
 * Globe — a dotted Earth that turns, with a marker per server.
 *
 * Ported from Originkit `globe` (preset `base`). three.js: land sampled off a
 * raster of Natural Earth into an InstancedMesh of dots, coastlines as tube
 * meshes, a 15° graticule, and markers placed by latitude and longitude.
 *
 * Four things differ from the source, each for a reason:
 *
 *  - **The land no longer comes off the network.** Upstream fetched
 *    `ne_50m_land.json` from `raw.githubusercontent.com` inside the mount
 *    effect: 2.76 MB from a third party, every time a customer opened the
 *    screen, with no result if they were offline. The data is baked into
 *    `globe-land-110m`, imported lazily so it lands in its own chunk.
 *  - **Dragging works with a finger.** The source bound `mousedown` on the
 *    canvas and `mousemove`/`mouseup` on `document` — mouse only. This runs
 *    inside a Telegram mini app, where that means the globe could not be turned
 *    at all. Pointer events cover mouse, touch and pen through one path, and
 *    pointer capture keeps a drag alive past the canvas edge without the
 *    document-level listeners, which leaked when unmounted mid-drag.
 *  - **The scene is no longer rebuilt on every render.** `dots` and
 *    `markerConfig` sat in the effect's dependency array while their defaults
 *    were object literals inside the destructuring, so omitting either prop
 *    produced a fresh object each render and tore down and rebuilt the whole
 *    WebGL scene. The defaults are hoisted, and the effect depends on the
 *    values inside them.
 *  - **No English on screen.** The upstream failure branch rendered its own
 *    message. The cabinet is bilingual and this component knows nothing about
 *    the reader's language, so it renders nothing and leaves the wording to the
 *    caller.
 *
 * Natural Earth is public domain; see `globe-land-110m` for provenance.
 */
import {
    Scene,
    PerspectiveCamera,
...
/**
 * Hoisted so that omitting the prop does not hand the effect a new object on
 * every render. As inline defaults these rebuilt the scene continuously.
 */
...
export default function Globe({
    ...
    /**
     * Markers arrive once, when the server list loads. Keying on their content
     * rather than the array identity keeps a caller's inline array from
     * rebuilding the scene on every render, while still redrawing when the set
     * actually changes.
     */
    const markersKey = markerConfig.markers
        .map((m) => `${m.lat},${m.lng}`)
        .join(';');
    ...
    useEffect(() => {
        ...
        // Without this the browser claims the gesture and pans the sheet
        // instead of turning the globe.
        ...
        const loadWorldData = async () => {
            try {
                setIsLoading(true);
                // Its own chunk: this screen opens on a double tap, so the
                // land has no business in the bundle that paints the card.
                const landFeatures = (await import('./globe-land-110m'))
                    .LAND_110M;
                // React runs the cleanup synchronously; this continuation then
                // resumes against a torn-down run. Without the check it
                // tessellates land into a dead scene, calls `render()` on a
                // disposed renderer — which re-primes the context it was meant
                // to release — and writes styles onto a canvas already removed
                // from the DOM. Opening the sheet and closing it before the
                // 88 KB chunk arrives is the NORMAL case on a phone.
                if (disposed) return;
        ...
        const handlePointerMove = (event: PointerEvent) => {
            ...
            // Hover pauses the spin, which only means anything with a cursor.
            // A touch reports itself as hovering the moment it lands, so
            // without this guard the globe would stop dead on first contact.
        ...
        return () => {
            ...
            // Every geometry and material this effect created, then the
            // context itself. `dispose()` alone frees three.js's caches and
            // leaves the WebGL context alive until GC collects the canvas —
            // and iOS gives a render process about sixteen, one of which the
            // subscription card behind this sheet is already holding. Opening
            // and closing the sheet repeatedly stranded one per open, and once
            // the process passes the limit WebKit drops the OLDEST context,
            // which by then is the card's own and is not recoverable.
    ...
    // The land is bundled now, so this can only be a genuine renderer failure.
    // Nothing is drawn and nothing is said: the caller owns the wording, in the
    // reader's language.
    if (error) {
        return <div style={containerStyle} aria-hidden="true" />;
    }
