/**
 * shadcn/ui Chart wrapper for Recharts v3.
 *
 * Source: https://ui.shadcn.com/docs/components/chart
 *
 * Provides a typed `ChartContainer` that:
 *   • emits per-series CSS variables (`--color-<key>`) from `config.color`
 *     so charts can use `fill="var(--color-foo)"` without hardcoding hexes.
 *   • wraps Recharts' `ResponsiveContainer` so charts size correctly.
 *   • ships matching Tooltip / Legend helpers (`ChartTooltipContent`,
 *     `ChartLegendContent`) styled with the rest of the theme.
 */
