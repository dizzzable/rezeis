 ...
 * as it is.
 *
 * `buttonTargetProblem` is also what every form that saves a button's target
 * checks it with before saving — «Кнопки бота» and the main-menu constructor
 * (`menuButtonTargetProblem`), a notification's buttons, a screen's — and what
 * the server refuses one with wherever it is saved: the same check on both
 * sides.
 */
