...
import { Megaphone, Info, Plus, Copy, Archive, BarChart3, Check } from 'lucide-react'
...
export default function AdvertisingPage() {
  ...
  const requests = useQuery({
    ...
    queryFn: () => listAdRequests('PENDING'),
  })
...
function PlacementTile({ placement }: { placement: AdPlacement }) {
  ...
  const copyLink = async () => {
    try {
      await navigator.clipboard.writeText(placement.links.botStart)
      setCopied(true)
...
function RequestRow({ id }: { id: string }) {
  const { t } = useTranslation()
  const queryClient = useQueryClient()
  const requests = useQuery({
    queryKey: ['admin', 'advertising', 'requests'],
  ...
  const approve = useMutation({
    mutationFn: () => approveAdRequest(id),
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-md border p-2 text-sm">
      ...
      <span className="text-xs text-muted-foreground">
        {request.platforms.map((p) => t(`advertisingPage.platforms.${p}`)).join(', ')}
      </span>
      <span className="text-xs text-muted-foreground">
        {t('advertisingPage.requests.proposedWindow', { days: request.proposedWindowDays })}
      </span>
      <div className="ml-auto flex gap-2">
        <Button size="sm" className="h-7" onClick={() => approve.mutate()} disabled={approve.isPending}>
          {t('advertisingPage.requests.approve')}
        </Button>
        <Button size="sm" variant="outline" className="h-7" onClick={() => reject.mutate()} disabled={reject.isPending}>
...
function NewPlacementDialog({ campaignId }: { campaignId: string }) {
  ...
  const labelWith = (label: string, hint: string): ReactNode => (
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <Label>{labelWith(t('advertisingPage.placement.ownerLabel'), t('advertisingPage.help.ownerType'))}</Label>
          ...
          <div className="space-y-1.5">
            <Label>{labelWith(t('advertisingPage.placement.windowLabel'), t('advertisingPage.help.attributionWindow'))}</Label>
            <Input type="number" min="1" max="365" value={windowDays} onChange={(e) => setWindowDays(e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label>{labelWith(t('advertisingPage.placement.bonusLabel'), t('advertisingPage.help.signupBonus'))}</Label>
