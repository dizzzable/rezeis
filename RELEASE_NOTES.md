# Rezeis Admin v0.9.5.21

Расширенный фон приложения, стартовые баннеры и фикс шумного 500 в логах. Парный релиз с reiwa v0.9.5.18.

### 🎨 Фон приложения — теперь не только анимация
- **«WEB Reiwa» → секция «Фон приложения» переработана в выбор режима:**
  - **Нет** — обычный цвет фона (как раньше).
  - **Градиент** — статичный CSS-градиент: пресеты + визуальный конструктор + «Сгенерировать из основного цвета» + ручной ввод.
  - **Текстура** — статичный тайловый узор (точки, сетка, диагональ, крестики, волны, карбон, треугольники, шум) с настройкой цвета узора/фона, размера и прозрачности + «Применить цвета бренда».
  - **Анимация** — прежний живой WebGL-эффект.
- Статичные режимы — чистый CSS (inline-SVG), не нагружают устройство и не мерцают. Аддитивно, без миграции БД; старые настройки читаются без потерь.

### 🖥️ Стартовый баннер
- При запуске API и воркера в логи выводится аккуратный баннер с названием, версией, ролью процесса и автором (**dizzzable**) — удобно видеть, что и какой версии поднялось (в духе баннера Remnawave). Отключается через `NO_COLOR`.

### 🐞 Исправления
- **Убран шумный `500` в логах на `PATCH /api/internal/user/language`.** Когда reiwa присылает язык нового пользователя ещё до его регистрации (а также для заблокированных регистраций), записи в БД ещё нет — теперь это отдаётся как чистый `404`, а не как необработанная ошибка Prisma `P2025`.

---

# Rezeis Admin v0.9.5.20

Мгновенная применяемость тем, настраиваемый фон приложения и устойчивость reiwa к перезагрузкам. Парный релиз с reiwa v0.9.5.17.

### Темы и фон
- **Изменения брендинга применяются сразу.** Сохранение брендинга/оформления в панели теперь шлёт reiwa инвалидацию (`reiwa.branding.invalidate`), и edge сбрасывает кэш `public-config` — новая тема видна на следующей загрузке кабинета без ожидания TTL (~60с). Открытый Mini App дополнительно обновляет брендинг при возврате на вкладку (throttled).
- **Новая настройка «Фон приложения».** В «WEB Reiwa» добавлена секция: анимированный фон за всем кабинетом (а не только под карточкой), переиспользует библиотеку эффектов карточки, с прозрачностью и предпросмотром. «Нет» → обычный цвет. Рендерится один раз (один WebGL-контекст) — не нагружает устройство. Аддитивно, без миграции БД.

### Mini App
- **Фикс «догруза» анимации фона при нескольких подписках.** Живой WebGL-эффект держит теперь только активная карточка; остальные показывают статичный градиент, а эффект плавно проявляется (мобильные браузеры ограничивают число WebGL-контекстов ~8 — это и вызывало мерцание/недогруз).

---

# Rezeis Admin v0.9.5.19

Платный триал в каталоге и анимированные премиум-эмодзи в панели. Парный релиз с reiwa v0.9.5.16.

### Платежи / триал
- **Платный пробный тариф теперь виден в каталоге и покупается.** Каталог (`/api/internal/catalog/plans`) стал контекстным: принимает `userId`/`telegramId`, и план с доступностью «Триал» + типом оплаты «Платно» теперь отображается в «Тарифах» Mini App и доступен к покупке. Заодно исправлена скрытая ошибка валидации `userId` (`@IsUUID` → строка/CUID) и теперь корректно показываются планы NEW/EXISTING/INVITED залогиненным пользователям. Цена платного триала берётся из блока «Цены и сроки» (как у обычного плана).

### Бот (админка) — анимированные эмодзи
- **Премиум-эмодзи показываются «как в Telegram».** Импорт пака теперь скачивает и хранит сам анимированный `.webm` (VP9), а не только статичный thumbnail; в превью редактора текстов, на странице эмодзи-паков и в пикерах эмодзи теперь играет реальная анимация (Lottie из `.tgs` или `.webm`-видео), а не статичная картинка. Сетка паков раньше была захардкожена на статику — пофикшено.
  - Чтобы у **уже импортированных** паков появилась webm-анимация, переимпортируйте их (старые хранят только thumbnail). TGS-эмодзи анимируются сразу.
...
### Бот (админка) — эмодзи и видимость
- **Пикер эмодзи везде с переводом.** Метки пикера (вкладки «Стандартные»/«Кастомные», поиск) жили в lazy-бандле страницы рассылок и в редакторе текстов бота показывались сырыми ключами. Перенесены в постоянный namespace — переводятся в любом месте.
- **Сильно расширен стандартный набор юникод-эмодзи** (лица, жесты, символы, объекты + новые категории: животные, транспорт/спорт, еда/природа).
- **Живой предпросмотр кастомных эмодзи.** Под полем текста (и EN-полем) `:slug:` рендерится картинкой эмодзи из пака — оператор видит результат, а не шорткод.
- **Кастомные эмодзи доезжают до бота.** Библиотека кастом-эмодзи (`:slug:` → `custom_emoji_id` + fallback-глиф) теперь отдаётся боту в bot-config — reiwa рендерит их как premium-эмодзи в приветствии (нужен `custom_emoji_id` у эмодзи; иначе показывается fallback-глиф).
- **Флаг «Использовать в боте» теперь работает.** Скрытый текст (и его EN-версия) исключается из переводов → бот отдаёт встроенный текст по умолчанию. Скрытое приветствие тоже падает на дефолт.

---

# Rezeis Admin v0.9.5.16

EN-локализация текстов бота и редактируемый стартовый экран. Парный релиз с reiwa v0.9.5.14 (доставка переводов + фикс логотипа в Mini App + экран смены пароля).

### Бот (админка) — английская версия каждого текста
- **Тумблер «Английская версия» в редакторе текстов бота.** Включаете — открывается второе поле (с тем же выбором эмодзи), куда вставляете уже готовый перевод. Пользователю с английской локалью бот покажет EN-вариант, остальным — базовый (RU); если перевода нет — встроенный текст по умолчанию. Работает для всех текстов, включая приветствие `bot.welcome_message`.
- **Без миграции БД.** Перевод хранится в строке-сестре `<ключ>@en`: пара пишется одной транзакцией, `@en`-строки скрыты из таблицы и редактируются только через свой базовый текст. Доставка в reiwa — через существующий механизм переводов (суффикс `.en`), так что подхватывается даже уже задеплоенным ботом.

### Бот (админка) — стартовый экран теперь работает
- **Тумблер «Стартовый экран» в Bot Studio наконец влияет на `/start`.** Если опубликован флоу с корневым (`isRoot`) экраном — его текст (RU/EN) становится приветствием, а его кнопки выводятся над основным меню (кабинет/пригласить/триал сохраняются). Раньше корневой экран не использовался вообще.

---

# Rezeis Admin v0.9.5.15

Триал-фикс, выбор эмодзи в текстах бота и фикс загрузки логотипа (data:image).

### Подписки
- **Бесплатный триал больше нельзя продлить.** Если в конфигураторе план-триал помечен «бесплатно», действие «Продлить» теперь недоступно (warning `TRIAL_FREE_NOT_RENEWABLE`) — пользователя ведём на апгрейд. Гейт стоит в quote-сервисе, поэтому закрывает и кабинет/бота, и серверный renewal-чекаут. Платный триал продлевается как прежде. (Апгрейды тарифов перепроверены: доступны только для планов с настроенными `upgradeToPlanIds` и только на указанные цели.)

### Брендинг
- **Логотип через `data:image/...;base64,...` теперь сохраняется.** Лимит длины поля логотипа (а также `cardLogoUrl`/`cardPattern`) был 8192 символа — реальный PNG/SVG в base64 в него не влезал, и сохранение падало с «Invalid input». Лимит поднят до 512 КБ на стороне SPA и бэкенда.

### Бот (админка)
- **Выбор эмодзи в редакторе текстов бота.** В диалогах редактирования/создания текста добавлен пикер (вставка по курсору): вкладка Standard (юникод, набор расширен) + вкладка Custom (премиум/кастомные эмодзи из реестра).
- **Скролл пикера** колесом мыши исправлен (в т.ч. внутри диалога редактирования текста).

---

# Rezeis Admin v0.9.5.14

Новый платёжный шлюз CryptoPay (@CryptoBot), очистка профилей по триалам, страница системных событий и пачка фиксов.

### Платежи — CryptoPay (@CryptoBot Crypto Pay API)
- **Добавлен шлюз `CRYPTOPAY`** в каталог: создание счёта (`createInvoice`), приём вебхука `invoice_paid` с проверкой подписи `HMAC-SHA256(body, SHA256(api_token))` по заголовку `crypto-pay-api-signature`, нормализация статусов (`paid`→оплачено, `active`→ожидание, `expired`→отменено) и идемпотентный дедуп через существующий inbox.
- Оплата в крипто-активах (USDT по умолчанию, плюс TON/BTC/ETH/LTC/USDC/TRX). Настройка в админке: «Платёжные шлюзы» → CryptoPay → `Crypto-Pay-API-Token`. В приложении @CryptoBot включить Webhooks на `/api/v1/payments/webhooks/CRYPTOPAY`.
- Миграция аддитивная (новое значение enum), существующие данные не затрагиваются; по умолчанию шлюз выключен.

### Подписки — очистка профилей Remnawave (trial-aware)
- **Worker-крон чистит панельные профили истёкших подписок** (освобождает место на панели), но **сохраняет локальную строку подписки** — учёт пробных подписок (`isTrial` + `TrialGrant` + платные `Transaction`) не теряется, повторно бесплатный триал получить нельзя.
- При удалении пользователя панельные профили его подписок удаляются best-effort (не блокируют удаление). Удаление профиля на панели через `DELETE` теперь трактует 404 как успех (не зацикливается).

### Система событий
- **Новая вкладка «Системные события»** в журнале аудита — лента событий `SystemEventsService` (создание подписок, платежи, ошибки) с фильтром по уровню.
- **Резервная доставка событий в Telegram**: если основная доставка выключена/без Chat ID, события уходят в личку dev/оператора (`devChatId` на экране «Доставка в Telegram»), а не теряются.

### Фиксы
- **Тарифы**: исправлен 400 при добавлении «доступных улучшений» — `upgradeToPlanIds`/`replacementPlanIds`/`allowedUserIds` валидировались как UUID, хотя ID являются CUID.
- **Шаблон имени профиля**: исправлен 400 при сохранении — `profileNaming` теперь типизирован в branding-настройках и сохраняется через `PATCH /admin/settings/branding` (раньше SPA слала его на platform-эндпоинт).
- **YooKassa**: в UI шлюза и README поправлен путь вебхука на корректный `/api/v1/payments/webhooks/YOOKASSA`.
...
# Rezeis Admin v0.9.5.13

Фикс: удаление пользователя больше не падает с 400.

### Пользователи
- **Удаление пользователя теперь снимает RESTRICT-зависимые записи в транзакции.** Три связи на `User` объявлены `onDelete: Restrict` (финансовая/кредитная история): `Transaction`, `PromocodeActivation`, `ReferralReward`. Если у пользователя есть хоть одна такая запись, `prisma.user.delete` падал с нарушением внешнего ключа (P2003), которое в админке выглядело как «Ошибка удаления» (HTTP 400). Теперь `DELETE /admin/users/:id` сначала удаляет эти записи (удаление транзакций каскадит их позиции, снимая Restrict и с подписок), затем самого пользователя — остальное (подписки, web-аккаунт, триал-грант, рефералы, партнёр, тикеты) каскадится как и раньше.

> Профиль на панели Remnawave при удалении пользователя не трогается (как и прежде). При повторной активации триала тем же логином sync переиспользует существующий профиль по username (идемпотентность), так что повторный тест проходит корректно.

---

# Rezeis Admin v0.9.5.12

Премиум-эмодзи на кнопке пробного периода в боте.

### Бот / эмодзи
- **Добавлен seed-эмодзи `TRIAL`** в реестр (`DEFAULT_EMOJIS`) с премиум `tgEmojiId` (gift-глиф из TranslucentPack). Зелёная кнопка «Попробовать бесплатно» в боте теперь рендерится с премиум-эмодзи из коробки — reiwa резолвит иконку по ключам `TRIAL → GIFT → PROMO` из `botEmojis`, а раньше ни одного из них в реестре по умолчанию не было, поэтому показывался обычный `🆓`. При отсутствии Premium у владельца бота — корректный откат на unicode `🆓`. Оператор может переопределить эмодзи в админке («Эмодзи», ключ `TRIAL`).
- Изменение на стороне rezeis; reiwa подхватит обновлённую карту `botEmojis` при следующем обновлении конфига (TTL/инвалидация). reiwa-релиз не требуется.

---

# Rezeis Admin v0.9.5.11

Фикс: heartbeat версии reiwa больше не отлетает с 400.

### Проверка обновлений
- **DTO `/api/internal/system/reiwa-version` принимает 4-сегментную версию** (`MAJOR.MINOR.PATCH.BUILD`, напр. `0.9.5.11`). Раньше regex допускал только 3 сегмента (`x.y.z`), поэтому reiwa со своей 4-сегментной версией репортил heartbeat и получал `400 BAD_REQUEST` — баннер «доступно обновление» для reiwa мог показывать устаревшую версию. `compareSemver` 4-й сегмент уже понимал — синхронизировали валидацию входа.
...
### Пробные подписки
- **Активация триала немедленно ставит задачу синхронизации профиля в очередь (BullMQ).** Раньше `grantTrial` создавал только PENDING-строку `ProfileSyncJob`, но (в отличие от платёжного пути и админской кнопки «Выдать триал») **не вызывал `enqueue`** — профиль на панели Remnawave создавался лишь cron-свипом `sweepAndRecover` раз в 5 минут. Из-за этого свежеактивированная пробная подписка в кабинете показывала сырой `id` подписки вместо имени профиля, без трафика и устройств, пока не отрабатывал свип. Теперь триал ведёт себя как обычная покупка — профиль появляется за секунды. Cron остаётся резервным механизмом восстановления.

Симптом, который это чинит: карточка пробной подписки показывала `cmqcym4me004k01jgyqshzk2u` (id), «Нет устройств» и без трафика, хотя сквад в тарифе был назначен.
...
### Web-кабинет / онбординг
- **Новый эндпоинт `POST /api/internal/web-auth/claim`** — привязывает `WebAccount` (логин + пароль) к **уже существующему** пользователю по его `reiwa_id`. Это серверная опора обязательного первого входа: Telegram-пользователь, у которого есть `User`, но нет `WebAccount`, задаёт логин и пароль — и дальше может зайти в кабинет из любого браузера без Telegram. Эндпоинт никогда не создаёт нового пользователя и привязывает данные только к аккаунту вызывающего (userId берётся из его WebSession в reiwa), поэтому угнать чужой аккаунт нельзя. Конфликты обрабатываются как при регистрации: `409` (логин занят / аккаунт уже привязан), `400` (невалидный логин/пароль).

Бэкенд переиспользует существующую политику логинов и хэширование пароля (scrypt), миграции БД не требуются: «привязан» = существование `WebAccount`. Полное покрытие новыми unit-тестами.

---

# Rezeis Admin v0.9.5.6

Критический фикс кэша: одноразовые токены и cache-фичи реально работают в проде.

### Redis-кэш
- **`RawCacheService` подключается по дискретным `REDIS_*`**, а не только по `REDIS_URL`. Прод-compose задаёт `REDIS_HOST/PORT/NAME/PASSWORD` (без `REDIS_URL`), из-за чего кэш молча становился no-op: `set`/`get` ничего не делали. Это ломало всё, что живёт в этом кэше — bot-signin magic-link токены (вход в кабинет/Mini App по Telegram), временные пароли и т.п. Теперь URL собирается из тех же `REDIS_*`, что и у остальных потребителей.

Симптом, который это чинит: вход в Telegram Mini App падал с 401 (`signin token consume returned null`) — токен «выпускался» в неинициализированный кэш и не находился при размене.
