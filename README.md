...
<p align="center">
  <a href="https://github.com/dizzzable/rezeis/releases/latest"><img src="https://img.shields.io/badge/version-0.2.14-blue" alt="Version" /></a>
...
## ✨ Что нового в v0.2.14

Большая итерация по реферальной программе: новый таб «Аналитика», иконки и фильтры на всех табах, синхронизация контрактов фронт↔бэк, фикс CSP, audit-trail для наград, bulk issue.

### Аналитика (новый таб)

Полноценная страница метрик с переключателем 7d/30d/90d. Что внутри:

- **Воронка конверсии** — 4 этапа с процентом конверсии: invites_created → consumed → qualified → rewards_issued. Сразу видно, на каком шаге теряются деньги.
- **Динамика по дням** (AreaChart) — приглашения, рефералы, квалификации и награды во времени. Гранулярность переключается с day на week автоматически на длинных диапазонах.
- **Топ-10 рефереров** — лидерборд с conversion rate и points earned за период.
- **Распределение наград** (PieChart) — POINTS vs EXTRA_DAYS × issued/pending/revoked.
- **Источники приглашений** (LineChart) — TELEGRAM/WEB/MANUAL/UNKNOWN.

Бэкенд эндпоинты: `/admin/referrals/analytics/{funnel,timeseries,top-referrers,reward-distribution,source-breakdown}`. Time-series считается на стороне Postgres через `date_trunc` + `generate_series`, без подгрузки сырых строк в JS.

### Иконки и UX

- Уровень реферала: `Crown` для L1 (золото), `Star` для L2/L3 (серебро/бронза).
- Источник: `MessageCircle` (Telegram), `Globe` (Web), `UserPlus` (Manual), `Link` (Unknown).
- Тип награды: `Coins` для POINTS, `CalendarPlus` для EXTRA_DAYS.
- Статусы invite: иконка + цветовая маркировка `active/expired/revoked/consumed`.
- KPI-карточки с иконками в углу.
- Поиск по таблицам (имя, username, telegram id, токен).
- Фильтры: уровень / статус квалификации (Рефералы), статус (Приглашения), тип + статус (Награды).
- Кнопка copy-to-clipboard на токенах invite.

### Bulk операции

В табе «Награды» — чекбоксы и кнопка «Выдать (N)». Бэкенд `POST /admin/referrals/rewards/bulk-issue` обрабатывает до 500 ID за один запрос, возвращает счётчики `{issued, skipped, failed}` плюс массив ошибок по проблемным ID. Транзакция на каждую отдельная, идемпотентность сохраняется.

### Audit-trail для наград

Миграция `20260524201400_referral_rewards_audit` добавляет в `referral_rewards`:

- `issued_by` (cuid) — кто выдал.
- `granted_by` (cuid) — кто создал вручную.
- `revoked_at` + `revoke_reason` — отзыв награды (для refund-flow в будущем).

Существующие строки получают `null` (легаси), новые операторские действия записывают актора. Два индекса (`issued_by`, `granted_by`) для отчётов «активность админа».

### Синхронизация контракта со SPA

Несколько эндпоинтов фронт ожидал, бэкенда не было — половина страницы рендерилась пустой. Сделано:

- `GET /admin/referrals/stats` теперь отдаёт `{invites, referrals, qualifiedReferrals, rewards, issuedRewards}` (плюс старые поля для совместимости).
- `GET /admin/referrals/rewards` — список наград с фильтрами (`type`, `issued`, `userId`, `referralId`, пагинация).
- `POST /admin/referrals/rewards` — manual grant (с проверкой что user — часть ребра referral).
- `POST /admin/referrals/rewards/:id/issue` — apply effect + mark issued (POINTS → User.points, EXTRA_DAYS → продлить подписку).
- `POST /admin/referrals/rewards/bulk-issue` — bulk.
- `POST /admin/referrals/rewards/:id/revoke` — отзыв pending награды.
- `POST /admin/referrals/attach` — telegram-id-friendly attach (резолв → cuid).
- `POST /admin/referrals/invites/:id/revoke` — alias к `DELETE` для совместимости со SPA.
- `GET /admin/settings/referral` + `PATCH /admin/settings/referral` — settings формы (раньше PATCH 404'ил, форма не сохранялась).

### CSP fix

`helmet()` без параметров блокировал `unsafe-eval`, что ломало vendored chunk с `@tanstack/react-query`/`zod` (используют `new Function` под капотом). Console спамил `Content Security Policy of your site blocks the use of 'eval'`. Снято: CSP отключён, остальные защиты (HSTS, X-Frame, X-Content-Type-Options) сохранены.

### Real-time

Глобальный `useRealtimeUpdates` уже маппит `referral.qualified` и `referral.reward_issued` на ключ `['admin', 'referrals']`. Все queries на странице используют этот префикс — данные обновляются автоматически без ручной подписки.

### Файлы

Backend:
- `rezeis-admin/src/main.ts` — отключение CSP в helmet.
- `rezeis-admin/src/modules/referrals/services/admin-rewards.service.ts` — новый сервис (list / grant / issue / bulk-issue / revoke).
- `rezeis-admin/src/modules/referrals/services/admin-referral-analytics.service.ts` — новый сервис (5 endpoint-ов).
- `rezeis-admin/src/modules/referrals/services/referrals.service.ts` — расширен `getStats()` под SPA-форму.
- `rezeis-admin/src/modules/referrals/controllers/admin-referrals.controller.ts` — переписан, добавлены 12 новых routes.
- `rezeis-admin/src/modules/referrals/dto/{create-reward,bulk-issue-rewards,list-rewards-query,admin-attach-referrer,analytics-range-query}.dto.ts` — DTO.
- `rezeis-admin/src/modules/referrals/interfaces/{admin-rewards,admin-referral-analytics,referral}.interface.ts` — типы.
- `rezeis-admin/src/modules/settings/{controllers/settings,services/settings}.{controller,service}.ts` — `GET/PATCH /admin/settings/referral`.
- `rezeis-admin/prisma/migrations/20260524201400_referral_rewards_audit/migration.sql` + `prisma/schema.prisma` — audit columns.

Frontend:
- `rezeis-admin/web/src/features/referrals/referrals-page.tsx` — переписан.
- `rezeis-admin/web/src/features/referrals/referrals-analytics-tab.tsx` — новый таб.
- `rezeis-admin/web/src/features/referrals/referrals-icons.ts` — иконки + tone мап.
- `rezeis-admin/web/src/i18n/{ru,en}.ts` — расширены.

### Migrating from 0.2.13

Без breaking changes для конечных пользователей. Бэкенд применит миграцию `20260524201400_referral_rewards_audit` на старте контейнера (4 nullable колонки + 3 индекса, безопасно).
...
## ✨ Что нового в v0.2.13

Хотфикс backend-эндпоинтов, на которых SPA получала 500/404. После v0.2.12 React-loop ушёл, и оставшиеся ошибки в console стали хорошо видны — вот они и разобраны.

### Что чинили

1. **`GET /api/admin/referrals` 500** — `referrals.service` указывал в `select` несуществующее поле `login` у модели `User`. Prisma бросала `PrismaClientValidationError`, фронт страницы рефералов не получал список. Поле заменено на корректное `username` (которое реально есть в схеме User), интерфейс `ReferralUserSummaryInterface` синхронизирован.
2. **`GET /api/admin/subscriptions?limit=50` 404 + `GET /api/admin/subscriptions/stats` 404** — у `AdminSubscriptionsController` не было read-эндпоинтов. Frontend ожидал список подписок и сводную статистику, но контроллер обслуживал только `POST /action-policy` и `POST /quote`. Добавлен новый read-only сервис `AdminSubscriptionsListService` и два метода в контроллер: `GET /` и `GET /stats`.

### Детали реализации

- `AdminSubscriptionsListService` живёт в том же модуле, но отделён от `SubscriptionQuoteService`, чтобы read-операции не цеплялись за тяжёлый граф зависимостей quote-сервиса.
- Список фильтруется по `status` (enum `SubscriptionStatus`) и `isTrial`, лимит до 500, упорядочен по `createdAt DESC, id DESC`.
- Статистика собирает `total`, `byStatus` (через `groupBy`), `trialCount` и `expiringIn7d` (ACTIVE-подписки с `expiresAt` в окне 7 дней) одним `Promise.all`.
- Отдаются оба поля `expireAt` и `expiresAt` — frontend пока ждёт `expireAt`, новые места могут пользоваться корректным `expiresAt`.
- DTO с `class-validator` (`IsEnum`, `IsBooleanString`, `Type(() => Number)`) — стандартный паттерн остальных query-DTO модуля.

### Tests

Контроллер-тесты обновлены под новые сигнатуры. Pre-existing breakage в `referrals.controllers.spec.ts` (тест ссылался на методы, удалённые из контроллеров до этого релиза) попутно пофиксен.
...
- `rezeis-admin/src/modules/subscriptions/controllers/admin-subscriptions.controller.ts` — `GET /` + `GET /stats`.
- `rezeis-admin/src/modules/subscriptions/services/admin-subscriptions-list.service.ts` — новый сервис.
- `rezeis-admin/src/modules/subscriptions/dto/list-subscriptions-query.dto.ts` — новый DTO.
- `rezeis-admin/src/modules/subscriptions/interfaces/admin-subscriptions-list.interface.ts` — interface'ы для list/stats.
- `rezeis-admin/src/modules/subscriptions/subscriptions.module.ts` — регистрация сервиса.
- `rezeis-admin/test/admin-subscriptions.controller.spec.ts` — обновлён под новый конструктор.
- `rezeis-admin/test/referrals.controllers.spec.ts` — переписан под актуальные методы контроллеров.
...
## ✨ Что нового в v0.2.12

Hotfix-релиз: устранён бесконечный render-loop в `QuickSearchOverlay`, который рассыпал React Error #301 по всем страницам админки.

### Корневая причина React Error #301 на всех страницах

После v0.2.11 пользователи продолжали ловить `Minified React error #301` ("Too many re-renders") при заходе на любую страницу. Локализовать удалось только после reproduce'а в playwright под `demoadmin`: трассировка через sourcemap указывала на `<ErrorBoundary>` в `app/providers.tsx`, а виновник всегда был один — `<QuickSearchOverlay>`, отрендеренный глобально в `admin-shell.tsx`.

В компоненте использовался такой паттерн:
...
Дефолт `[]` при destructure'инге создаёт **новый литерал массива на каждом рендере**, пока `useQuery` возвращает `undefined`. Сравнение `results !== prevResults` поэтому всегда `true` → `setState` в render-фазе → ре-рендер → новый `[]` → снова `true` → infinite loop → React 18 кидает Error #301 и `<ErrorBoundary>` ловит его на всех страницах.

### Фикс

- Введён module-level `const EMPTY_RESULTS: SearchResult[] = []` со стабильной идентичностью.
- Деструктуризация заменена на `const { data, isFetching } = useQuery(...)` + явный fallback `const results = data ?? EMPTY_RESULTS`.

Это [рекомендованный React паттерн](https://react.dev/learn/you-might-not-need-an-effect#adjusting-some-state-when-a-prop-changes) "store previous in render": идентичность сравниваемого значения должна быть стабильной, иначе условие никогда не «успокаивается».

### Итог

- Глобальный `<QuickSearchOverlay>` больше не уходит в render-loop при загрузке любой страницы.
- `<ErrorBoundary>` остаётся на месте как страховка, но больше не срабатывает на каждом маршруте.
- Снимаются побочные ошибки от взаимодействия со сломанным деревом (например, "The width(-1) and height(-1) of chart" от Recharts).

### Migrating from 0.2.11

Без breaking changes. Стандартный `docker compose pull && docker compose up -d`. После деплоя пользователи получают свежий бандл при следующем визите без hard reload благодаря `Cache-Control` headers, включённым в v0.2.11.

### Файлы

- `rezeis-admin/web/src/components/quick-search/quick-search-overlay.tsx` — `EMPTY_RESULTS` стабильный fallback.
...
docker pull ghcr.io/dizzzable/rezeis:0.2.14

# Pin to a minor line (gets 0.2.x updates automatically)
docker pull ghcr.io/dizzzable/rezeis:0.2
```

Доступные теги: `latest`, `0.2.14`, `0.2`, `v0.2.14`, плюс `sha-<short>` для каждого коммита в `main`.
