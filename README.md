...
<p align="center">
  <a href="https://github.com/dizzzable/rezeis/releases/latest"><img src="https://img.shields.io/badge/version-0.9.7.63-blue" alt="Version" /></a>
...
Rezeis — продвинутая админ-панель для управления VPN-инфраструктурой с интеграцией [Remnawave Panel](https://github.com/remnawave/panel). Один интерфейс закрывает все операционные задачи: пользователи, подписки, платежи, ноды, мониторинг, антифрод, реферальная и партнёрская программы.
...
docker pull ghcr.io/dizzzable/rezeis:v0.9.7.63
```

Доступные теги: `latest` (последний выпуск — двигается только push'ем тега `v*`), `v0.9.7.63` (текущий тег релиза), `main` (текущая ветка `main`, ещё не выпуск), плюс `sha-<short>` для каждого собранного коммита. Для воспроизводимого развёртывания указывайте версию релиза, а не `latest`.
...
- **Bot Studio** — визуальный конструктор Telegram-бота на `@xyflow/react`: экраны, ветвления, кнопки, действия, баннеры и операторские тексты
- **Custom Emoji** — импорт и каталог паков Telegram Custom Emoji для бота, рассылок и интерфейса
- **Контентные поверхности** — FAQ с медиа, Web Landing Builder и конфигурация подключаемой subpage
- **Поддержка** — тикеты, запрос документов и управляемый AI-помощник; ключ провайдера остаётся в Rezeis
- **Growth-инструменты** — задания, рекламный кабинет и заявки партнёров наряду с промокодами, рефералами и партнёркой
- **Backup/Restore и импорты** — ручные и плановые бэкапы, восстановление, перенос конфигурации и импорт поддерживаемых внешних систем
- **Доступы и интеграции** — RBAC, API-токены, 2FA, OAuth/внешняя авторизация, IP allow/block-листы, webhooks и журнал аудита
...
### 📱 Клиентский контур Reiwa

Rezeis остаётся источником бизнес-правил, а Reiwa реализует клиентский кабинет, Mini App и бот. Из панели управляются тарифы, пробные условия, продления, доп-услуги, способы оплаты, тексты, FAQ, уведомления, партнёрские и рекламные сценарии; пользователь получает только разрешённые ему действия и контент.

- **Подписки и ограничения:** создание тарифов, пробные правила, лимиты активаций, устройства, трафик, автообновление и безопасные операции жизненного цикла
- **Платёжный путь:** каталог доступных шлюзов, сохранённые методы, покупка, продление, улучшение и доп-услуги проходят через единую серверную логику и webhook/reconciliation-контур
- **Единый вход:** Rezeis связывает Telegram-идентичность, web-учётную запись и внешнюю авторизацию; Reiwa использует это для бесшовного входа из Mini App и безопасной browser-сессии
- **События пользователю:** Rezeis генерирует шаблоны и события, Reiwa доставляет их в кабинет, PWA и Telegram в соответствии с настройками оператора
