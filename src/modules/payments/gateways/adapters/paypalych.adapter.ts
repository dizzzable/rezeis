/**
 * PayPalych (Pally) payment gateway adapter.
 *
 * Docs: https://pally.info/reference/api
 * Auth: Bearer token, HMAC-SHA256 webhook signature.
 *
 * Settings shape:
 *   shopId: string
 *   apiKey: string
 *   secretKey: string
 */

import { Injectable, Logger } from '@nestjs/common';
import type { Request } from 'express';
import * as crypto from 'node:crypto';
import type {
  GatewayCheckoutInput,
  GatewayCheckoutResult,
  IPaymentGateway,
  NormalizedWebhookEvent,
  WebhookEventStatus,
  WebhookVerifyResult,
} from '../gateway.interface';

const BASE_URL = 'https://pally.info/api/v1';

@Injectable()
export class PaypalychAdapter implements IPaymentGateway {
  readonly type = 'PAYPALYCH';
  private readonly logger = new Logger(PaypalychAdapter.name);

  private async apiPost<T>(path: string, body: object, settings: Record<string, unknown>): Promise<T> {
    const apiKey = settings['apiKey'] as string;

    const res = await fetch(`${BASE_URL}/${path}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${apiKey}`,
      },
      body: JSON.stringify(body),
    });

    if (!res.ok) {
      const text = await res.text();
      throw new Error(`PayPalych API error ${res.status}: ${text}`);
    }
    return res.json() as Promise<T>;
  }

  async createCheckout(input: GatewayCheckoutInput, settings: Record<string, unknown>): Promise<GatewayCheckoutResult> {
    const shopId = settings['shopId'] as string;

    const body = {
      shop_id: shopId,
      bill_id: input.paymentId,
      amount: input.amount,
      currency: 'RUB',
      name: input.description.slice(0, 255),
      success_url: input.successUrl,
      fail_url: input.failUrl,
      custom: input.paymentId,
    };

    const response = await this.apiPost<{
      success: boolean;
      link_url?: string;
      bill_id?: string;
      error?: string;
    }>('bill/create', body, settings);

    if (!response.success || !response.link_url) {
      throw new Error(`PayPalych checkout failed: ${response.error ?? 'unknown error'}`);
    }

    return {
      externalPaymentId: response.bill_id ?? input.paymentId,
      paymentUrl: response.link_url,
      raw: response,
    };
  }

  async verifyWebhook(req: Request, settings: Record<string, unknown>): Promise<WebhookVerifyResult> {
    const secretKey = settings['secretKey'] as string | undefined;
    if (!secretKey) return { valid: false, reason: 'PayPalych secretKey not configured' };

    const signature = req.headers['x-signature'] as string | undefined;
    if (!signature) return { valid: false, reason: 'Missing X-Signature header' };

    const rawBody = (req as Request & { rawBody?: Buffer }).rawBody?.toString('utf8')
      ?? JSON.stringify(req.body);

    const expected = crypto.createHmac('sha256', secretKey).update(rawBody).digest('hex');
    const valid = crypto.timingSafeEqual(Buffer.from(expected), Buffer.from(signature));
    return valid ? { valid: true } : { valid: false, reason: 'Invalid PayPalych webhook signature' };
  }

  async parseWebhook(req: Request): Promise<NormalizedWebhookEvent> {
    const payload = req.body as Record<string, unknown>;

    return {
      paymentId: String(payload['custom'] ?? payload['bill_id'] ?? ''),
      externalPaymentId: String(payload['bill_id'] ?? ''),
      status: this.mapStatus(String(payload['status'] ?? '')),
      amount: payload['amount'] ? parseFloat(String(payload['amount'])) : undefined,
      currency: String(payload['currency'] ?? 'RUB'),
      eventType: 'payment',
      raw: payload,
    };
  }

  private mapStatus(status: string): WebhookEventStatus {
    switch (status.toLowerCase()) {
      case 'success':
      case 'paid': return 'SUCCESS';
      case 'fail':
      case 'failed':
      case 'error': return 'FAILED';
      case 'cancel':
      case 'canceled': return 'CANCELED';
      case 'refund':
      case 'refunded': return 'REFUNDED';
      default: return 'PENDING';
    }
  }
}
